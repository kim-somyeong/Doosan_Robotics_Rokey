...
class YoloCameraApp:
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(2)  # 카메라 인덱스 설정 (필요에 따라 변경)
    ...
    def run_yolo(self):
        while self.running:
            ...
            detection_zone_x = int(width * 0.25)  # 좌측에서 1/4 지점
            detection_zone = (detection_zone_x, 0, width, height)  # 우측 3/4 영역
            ...
            for result in results:
                for box in result.boxes:
                    ...
                    # 객체의 절반 이상이 감시 영역에 들어왔는지 확인
                    box_center_x = (x1 + x2) / 2  # 바운딩 박스의 중앙 x 좌표
                    
                    if box_center_x >= detection_zone_x and score >= 0.9:
