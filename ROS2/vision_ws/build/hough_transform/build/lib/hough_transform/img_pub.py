...
class ImagePublisher(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(Image, "original_image", 10) # publisher 생성
        self.setup_timer()
    ...
    def setup_timer(self):
        ...
        self.timer = self.create_timer(0.1, self.publish_image) # publish_image라는 timer를 callback

    def publish_image(self):
        """Read an image from file and publish it."""
        cv_image = cv2.imread(self.image_path)
        if cv_image is None:    #cv_image가 없으면 error msg 출력
            self.get_logger().error(f"Failed to load image from {self.image_path}")
            return

        ros_image = self.bridge.cv2_to_imgmsg(cv_image, encoding="bgr8")    # 있으면 ros_image로 변환
        self.publisher.publish(ros_image)   # 변환된 것을 publish
